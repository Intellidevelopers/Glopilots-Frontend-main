...
const MultipleMap = ({ navigation }) => {
  ...
  const [textInputVisibility, setTextInputVisibility] = useState([true]); // Array to store the visibility of each TextInput
  ...
  const coordinates = [
    ...
    { id: 2, latitude: 37.7896386, longitude: -122.421646, title: 'Marker 2' },
    { id: 3, latitude: 37.785839, longitude: -122.41859, title: 'Marker 3' },
    { id: 4, latitude: 37.780807, longitude: -122.414216, title: 'Marker 4' },
    { id: 5, latitude: 37.776916, longitude: -122.411524, title: 'Marker 5' },
  ];
  ...
  const renderPolyline = () => {
    ...
    return <Polyline coordinates={polylineCoordinates} strokeColor="#3498db" strokeWidth={2} />;
  };
...
{textInputVisibility.map((isVisible, index) => (
  isVisible && (
    ...
    <TextInput
      ...
      value={additionalAddresses[index]}
      onChangeText={(text) => handleAdditionalAddressChange(text, index)}
      editable={false}
    />
    </TouchableOpacity>
  )
  ...
  onPress={() => {
    if (additionalAddresses.length < 5) {
      ...
      setTextInputVisibility([...textInputVisibility, true]);
    }
  }}
>
